...
// Store provides all funcs for queries and transactions
type Store struct {
	// queries only supports queries not transactions,
	// so we use it in store struct and add more functionality,
	// so store can do both queries and transcations (concept Composition)
	*Queries
	db *sql.DB
}
...
// contains input params required by a transfer transaction
...
// It creates a transfer record, add account entries, update accounts'
